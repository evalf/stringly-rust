...
/// Serializes an object to prettyprinted Stringly.
pub fn to_string_pretty<T>(value: &T) -> Result<String>
where
  T: Serialize,
{
  Ok(util::prettify(&value.serialize(Serializer)?))
}
...
impl ser::Serializer for Serializer {
  ...
  fn serialize_seq(self, _len: Option<usize>) -> Result<Self::SerializeSeq> {
    Ok(SerializeSequence { n: 0, output: String::new() })
  }
  ...
  fn serialize_tuple_variant(self, _name: &'static str, _variant_index: u32, variant: &'static str, _len: usize) -> Result<Self::SerializeTupleVariant> {
    // TODO: assert '{' and '}' not in `variant`
    Ok(SerializeVariantSequence { variant, n: 0, output: String::new() })
  }
...
impl ser::SerializeTupleVariant for SerializeVariantSequence {
  ...
  fn end(self) -> Result<Self::Ok> {
    Ok([self.variant.to_string(), util::protect_unconditionally(&self.output)].concat())
  }
}
