...
impl fmt::Display for Error {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    match *self {
      ...
      Error::NotABoolean => f.write_str("expected a boolean (`true`, `yes`, `false`, `no`; case insensitive)"),
      ...
      Error::IndentTooSmall { lineno } => write!(f, "line {}: indentation should be two or more space but got one", lineno),
      Error::UnmatchedUnindent { lineno } => write!(f, "line {}: unindent does not match any outer indentation level", lineno),
    }
  }
}
