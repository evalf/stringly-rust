use serde::de::{self, Deserialize, DeserializeOwned, DeserializeSeed, EnumAccess, MapAccess, SeqAccess, VariantAccess, Visitor};
...
/// Deserializes an object from prettyprinted Stringly.
pub fn from_str_pretty<T>(s: &str) -> Result<T>
where
  T: DeserializeOwned,
{
  T::deserialize(Deserializer::from_str(&util::deprettify(s)?))
}
...
impl<'de> de::Deserializer<'de> for Deserializer<&'de str> {
  ...
  {
    let mut iter = util::safesplit(self.input, ',');
    let v = visitor.visit_map(DeserializeMap { iter: &mut iter, value: None });
    match iter.next() {
      None => v,
      Some(_) => Err(Error::TooManyElements),
    }
...
impl<'de, 'b> SeqAccess<'de> for DeserializeSequence<'de, 'b> {
    ...
    T: DeserializeSeed<'de>,
  {
    match self.iter.next() {
      Some(s) => seed.deserialize(Deserializer::from_str(util::unprotect(s))).map(Some),
      None => Ok(None),
    }
  }
}
    ...
    match self.iter.next() {
      Some(s) => match util::safesplit_once(s, '=') {
        Ok((key, value)) => {
          ...
          seed.deserialize(Deserializer::from_str(util::unprotect(key))).map(Some)
        }
...
impl<'de> EnumAccess<'de> for DeserializeEnum<'de> {
  ...
  where
    V: DeserializeSeed<'de>,
  {
    Ok((seed.deserialize(Deserializer::from_str(self.variant))?, self))
  }
}
